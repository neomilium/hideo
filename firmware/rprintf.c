...
static char
__attribute__((progmem)) HexChars[] = "0123456789ABCDEF";
...
void
rprintfStrLen(char str[], unsigned int start, unsigned int len)
{
	register int	i = 0;
	...
	for (i = 0; i < len; i++) {
		/*
		 * print data out of the string as long as we haven't reached
		 * a null yet
		 */
...
void
rprintfNum(char base, char numDigits, char isSigned, char padchar, long n)
{
...
/* %c - character */
int
rprintf1RamRom(unsigned char stringInRom, const char *format,...)
{
	...
	va_start(ap, format);
	for (;;) {
		while ((format_flag = READMEMBYTE(stringInRom, format++)) != '%') {	/* Until '%' or '\0' */
			if (!format_flag) {
				va_end(ap);
				return (0);
			}
...
int
rprintf2RamRom(unsigned char stringInRom, const char *sfmt,...)
{
	...
	for (; READMEMBYTE(stringInRom, f); f++) {
		if (READMEMBYTE(stringInRom, f) != '%') {	/* not a format
								 * character */
...
int
atoiRamRom(unsigned char stringInRom, char *str)
{
	int		num = 0;;
