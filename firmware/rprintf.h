/**
 * @file rprintf.h
 * @brief printf routine and associated routines.
 *
 * File Name	: 'rprintf.h'
 ...
 * @par Overview
 * The rprintf function library provides a simplified (reduced) version of the
 * common C printf() function.    See the code files for details about which
 * printf features are supported.    Also in this library are a variety of
 ...
 *
 * @note All output from the rprintf library can be directed to any device or
 * software which accepts characters.    This means that rprintf output can be
 * sent to the UART (serial port) or can be used with the LCD display libraries
 * to print formatted text on the screen.
 *
 */
/* @{ */
...
void		rprintfu04(unsigned char data);	/* /< Print 4-bit hex number.
						 * Outputs a single hex
						 * character. */
void		rprintfu08(unsigned char data);	/* /< Print 8-bit hex number.
						 * Outputs two hex
						 * characters. */
void		rprintfu16(unsigned short data);	/* /< Print 16-bit hex
							 * number. Outputs four
							 * hex characters. */
void		rprintfu32(unsigned long data);	/* /< Print 32-bit hex
						 * number. Outputs eight hex
						 * characters. */
...
#define rprintf(format, args...)  		rprintf1RamRom(STRING_IN_ROM, PSTR(format), ## args)
 ...
 /*
  * / Called by rprintf() - does a more powerful printf (supports %d, %u, %o,
  * %x, %c, %s).
  */
 /* / Supports: */
 /* / - %d - decimal */
