 ...
 * necessary. This buffer is not dynamically allocated, it has a user-defined
 * fixed maximum size. This buffer is used in many places in the avrlib code.
 *
...
typedef struct struct_cBuffer {
	unsigned char  *dataptr;/* the physical memory address where the
				 * buffer is stored */
	unsigned short	size;	/* the allocated size of the buffer */
	unsigned short	datalength;	/* the length of the data currently
					 * in the buffer */
	unsigned short	dataindex;	/* the index into the buffer where
					 * the data starts */
}		cBuffer;
